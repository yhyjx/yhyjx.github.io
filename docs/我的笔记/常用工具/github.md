...
- 编写如下 `githubHosts` 放置到 `/usr/local/bin`环境变量中
- 通过`sudo chmod +x hostgithub `赋予系统权限
- 之后通过执行 `githubHosts -u` 进行更新 hosts ，hosts 地址采用`https://raw.hellogithub.com/hosts`可自行修改。
- 修改后如果未生效执行 `sudo dscacheutil -flushcache` 刷新 dns 缓存。
